...
const testDir = join(tmpdir(), "remembrance-test-handlers");
...
describe("writeMemory", () => {
  ...
  it("stores tags as a JSON array", () => {
    ...
    const row = db.prepare("SELECT tags FROM entries WHERE id = ?").get(id) as {
      tags: string;
    };
...
describe("updateMemory", () => {
  ...
  it("throws for an unknown id", () => {
    expect(() =>
      handlers.updateMemory({ id: "nonexistent-id", content: "New" }),
    ).toThrow();
  });
});
...
describe("forgetMemory", () => {
  ...
  it("sets deleted=1 in the database (entry still physically present)", () => {
    ...
    handlers.forgetMemory({ id });
    const row = db
      .prepare("SELECT deleted FROM entries WHERE id = ?")
      .get(id) as { deleted: number };
