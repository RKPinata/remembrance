...
```typescript
const existing = await memory.searchMemory({ query: 'auth strategy', scope: 'project' })
if (existing.length > 0) {
  await memory.updateMemory({ id: existing[0].id, content: newContent })
} else {
  await memory.writeMemory({ scope: 'project', type: 'decision', content: newContent })
}
```
...
```typescript
const states = await memory.readMemory({ scope: 'project', type: 'state', limit: 50 })
...
- Surface these to the user: *"This entry is 90 days old and unverified — still accurate?"*
